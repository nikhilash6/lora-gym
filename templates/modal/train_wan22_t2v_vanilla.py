"""
Wan 2.2 T2V A14B LoRA Training — Vanilla (No Lightning)
========================================================
Trains against the base DiT weights without any Lightning merge.
Use this for standard 30-50 step workflows.

Usage:
  1. Upload dataset:  python -m modal run train_wan22_t2v_vanilla.py::add_to_volume_datasets
  2. Train high:      python -m modal run train_wan22_t2v_vanilla.py::run_high
  3. Train low:       python -m modal run train_wan22_t2v_vanilla.py::run_low
...
app = App(name="wan22-t2v-vanilla")
...
def train(noise_level: str):
    ...
    print("=" * 60)
    print(f"  Wan 2.2 T2V A14B — {label} Expert — VANILLA")
    print(f"  Output: {name}")
    print("=" * 60)
    ...
    dit_file = f"split_files/diffusion_models/wan2.2_t2v_{'high' if is_high else 'low'}_noise_14B_fp16.safetensors"
    ...
    # Train
    min_ts, max_ts = ("875", "1000") if is_high else ("0", "875")
    train_cmd = [
