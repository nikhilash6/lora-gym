"""
Wan 2.2 I2V A14B LoRA Training — RunPod Edition (Vanilla / No Lightning)
=========================================================================
...
Usage:
  ...
  python /workspace/train_runpod_i2v_vanilla.py --noise_level high

  # Train low-noise expert
  python /workspace/train_runpod_i2v_vanilla.py --noise_level low
  ...
  # Custom config
  python /workspace/train_runpod_i2v_vanilla.py --noise_level high --lr 5e-5 --epochs 30 --dim 32

  # Resume from a specific checkpoint
  python /workspace/train_runpod_i2v_vanilla.py --noise_level high --resume_from /workspace/outputs/my-lora-e25.safetensors
...
def train(args):
    ...
    print("=" * 60)
    print(f"  Wan 2.2 I2V A14B — {label} Expert — VANILLA (RunPod)")
    ...
    # I2V boundary: 900 (T2V would be 875)
    if is_high:
    ...
    train_cmd = [
        ...
        "--task", "i2v-A14B",              # Wan 2.2 I2V MoE architecture
        "--dit", dit_path,
        "--vae", vae_path,
        "--t5", t5_path,
        "--dataset_config", DATASET_CONFIG,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Wan 2.2 I2V A14B LoRA Training — RunPod Edition (Vanilla / No Lightning)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Train high-noise expert with defaults
  python train_runpod_i2v_vanilla.py --noise_level high

  # Train low-noise with custom learning rate
  python train_runpod_i2v_vanilla.py --noise_level low --lr 5e-5

  # Resume from a specific checkpoint
  python train_runpod_i2v_vanilla.py --noise_level high --resume_from /workspace/outputs/my-lora-e25.safetensors

  # Full custom config
  python train_runpod_i2v_vanilla.py --noise_level high --lr 5e-5 --dim 32 --alpha 32 --epochs 30
        """
    )
