"""
Wan 2.1 T2V 1.3B LoRA Training — RunPod Edition
=================================================
Trains a character identity LoRA on a RunPod GPU pod using musubi-tuner.

Wan 2.1 1.3B is the smallest Wan model (~2.84 GB in bf16). It can run on
GPUs with as little as 12-16 GB VRAM without fp8 quantization. Great for
quick iteration and testing before scaling up to 14B.

Usage:
  cd /workspace/musubi-tuner
  python /workspace/train_runpod_wan21_t2v_1_3b.py

  # Custom config
  python /workspace/train_runpod_wan21_t2v_1_3b.py --lr 5e-5 --epochs 30 --dim 32

  # Resume from a specific checkpoint
  python /workspace/train_runpod_wan21_t2v_1_3b.py --resume_from /workspace/outputs/my-lora-e25.safetensors

Model downloads required (add to setup_runpod.sh or download manually):
  Comfy-Org/Wan_2.1_ComfyUI_repackaged:
    split_files/diffusion_models/wan2.1_t2v_1.3B_bf16.safetensors  (~2.84 GB)
    split_files/vae/wan_2.1_vae.safetensors
  Wan-AI/Wan2.1-I2V-14B-720P:
    models_t5_umt5-xxl-enc-bf16.pth

GPU requirements: 12+ GB VRAM (RTX 3060 and above)
"""
...
# --- Model files ---
MODEL_PATHS = {
    "dit":  f"{MODELS_DIR}/wan2.1_t2v_1.3B_bf16.safetensors",
...
def train(args):
    ...
    print("=" * 60)
    print(f"  Wan 2.1 T2V 1.3B (RunPod)")
    print(f"  Output: {name}")
    ...
    # =================================================================
    print("\n" + "=" * 60)
    print(f"  Step 3: Training Wan 2.1 T2V 1.3B LoRA...")
    ...
    train_cmd = [
        ...
        "--task", "t2v-1.3B",              # Wan 2.1 T2V 1.3B (single model)
        "--dit", dit_path,
        "--vae", vae_path,
        "--t5", t5_path,
        "--dataset_config", DATASET_CONFIG,
        ...
        "--vae_cache_cpu",
        # NOTE: No --fp8_base for 1.3B — model is small enough without it
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Wan 2.1 T2V 1.3B LoRA Training — RunPod Edition",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Train with defaults
  python train_runpod_wan21_t2v_1_3b.py

  # Custom learning rate and epochs
  python train_runpod_wan21_t2v_1_3b.py --lr 5e-5 --epochs 30

  # Resume from a checkpoint
  python train_runpod_wan21_t2v_1_3b.py --resume_from /workspace/outputs/my-lora-e25.safetensors

  # Full custom config
  python train_runpod_wan21_t2v_1_3b.py --lr 5e-5 --dim 32 --alpha 32 --epochs 30
        """
    )
