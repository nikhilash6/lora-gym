"""
Wan 2.2 I2V A14B LoRA Training — RunPod Edition (Lightning-Optimized)
=====================================================================
...
Usage:
  ...
  python /workspace/train_runpod_i2v_lightning.py --noise_level high --resume_from /workspace/outputs/my-lora-e25.safetensors
...
def train(args):
    ...
    print("=" * 60)
    print(f"  Wan 2.2 I2V A14B — {label} Expert (RunPod)")
    print(f"  Output: {name}")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Wan 2.2 I2V A14B LoRA Training — RunPod Edition",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Train high-noise expert with defaults
  python train_runpod_i2v_lightning.py --noise_level high

  # Train low-noise with custom learning rate
  python train_runpod_i2v_lightning.py --noise_level low --lr 5e-5

  # Train without Lightning merge
  python train_runpod_i2v_lightning.py --noise_level high --skip_lightning

  # Resume from a specific checkpoint
  python train_runpod_i2v_lightning.py --noise_level high --resume_from /workspace/outputs/my-lora-e25.safetensors

  # Full custom config
  python train_runpod_i2v_lightning.py --noise_level high --lr 5e-5 --dim 32 --alpha 32 --epochs 30
        """
    )
    ...
    parser.add_argument("--resume_from", default=None,
                        help="Path to a .safetensors LoRA checkpoint to resume from. "
                             "Uses --network_weights (weights only, no optimizer state). "
