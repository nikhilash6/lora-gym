"""
Wan 2.1 T2V 14B LoRA Training — RunPod Edition
================================================
Trains a character identity LoRA on a RunPod GPU pod using musubi-tuner.

Wan 2.1 uses a single DiT model (no dual noise experts like Wan 2.2).
This means simpler training — no noise_level argument, no timestep splitting,
and no Lightning LoRA merge.

Usage:
  cd /workspace/musubi-tuner
  python /workspace/train_runpod_wan21_t2v_14b.py

  # Custom config
  python /workspace/train_runpod_wan21_t2v_14b.py --lr 5e-5 --epochs 30 --dim 32

  # Resume from a specific checkpoint
  python /workspace/train_runpod_wan21_t2v_14b.py --resume_from /workspace/outputs/my-lora-e25.safetensors

Model downloads required (add to setup_runpod.sh or download manually):
  Comfy-Org/Wan_2.1_ComfyUI_repackaged:
    split_files/diffusion_models/wan2.1_t2v_14B_bf16.safetensors  (~28.6 GB)
...
OUTPUT_NAME = "annika-wan21-t2v-14b"
...
# --- Model files ---
MODEL_PATHS = {
    "dit":  f"{MODELS_DIR}/wan2.1_t2v_14B_bf16.safetensors",
...
def train(args):
    ...
    print("=" * 60)
    print(f"  Wan 2.1 T2V 14B (RunPod)")
    print(f"  Output: {name}")
    ...
    # =================================================================
    print("\n" + "=" * 60)
    print(f"  Step 3: Training Wan 2.1 T2V 14B LoRA...")
    ...
    train_cmd = [
        ...
        "--task", "t2v-14B",               # Wan 2.1 T2V 14B (single model)
        "--dit", dit_path,
        "--vae", vae_path,
        "--t5", t5_path,
        "--dataset_config", DATASET_CONFIG,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Wan 2.1 T2V 14B LoRA Training — RunPod Edition",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Train with defaults
  python train_runpod_wan21_t2v_14b.py

  # Custom learning rate and epochs
  python train_runpod_wan21_t2v_14b.py --lr 5e-5 --epochs 30

  # Resume from a checkpoint
  python train_runpod_wan21_t2v_14b.py --resume_from /workspace/outputs/my-lora-e25.safetensors

  # Full custom config
  python train_runpod_wan21_t2v_14b.py --lr 5e-5 --dim 32 --alpha 32 --epochs 30
        """
    )
